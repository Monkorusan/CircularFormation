...
と定義する。ここで、上式は、Stuart-Landau発振器を簡略化したものであり、下式は、蔵本モデルを簡略化したもの（全結合型を一方向結合型に緩和したもの）である。ただし、$B_i$ は、位相のバイアスを表す。

２輪移動型ロボット $i$ のダイナミクスは
$$
\begin{bmatrix}
\dot{r}_i \\
r_i\dot{\phi}_i \\
...
2輪移動型ロボットを図のように与えると、ベクトル場に追従するための制御入力は
$$
